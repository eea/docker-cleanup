...
build:
	docker build --rm -t eeacms/docker-cleanup .

run:
	docker run --env-file ./env.list --name docker-cleanup -v /var/run/docker.sock:/var/run/docker.sock:rw -v /var/lib/docker:/var/lib/docker:rw -d eeacms/docker-cleanup
